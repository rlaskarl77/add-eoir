...
SAVE_ROOT = Path(get_resources_path())
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if plots and batch_i < 3:
            f = save_dir / f'{task}_batch{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(im, targets, paths, f, names), daemon=True).start()
            f = save_dir / f'{task}_batch{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(im, output_to_target(out), paths, f, names), daemon=True).start()

    # Compute metrics
    stats = [np.concatenate(x, 0) for x in zip(*stats)]  # to numpy
    if len(stats) and stats[0].any():
        plot = (nc == len(names))   # Don't plot confusion matrix if # classes in training and testing doesn't match
        tp, fp, p, r, f1, ap, ap_class = ap_per_class(*stats, plot=plot, save_dir=save_dir, names=names)
        if person_only:
    ...
    if save_json and len(jdict):
        ...
        # ADD EOIR dataset
        if 'ADD' in dataset_name or 'kaist-rgbt' in dataset_name:
            anno_json = str(Path(data.get('path')) / Path(data.get(f'{task}_anno_json')))
        ...
        try:
            ...
            if eval_tod:
                ...
                from utils.cocoapi.pycocotools.cocoeval import COCOeval as COCOeval
...
def parse_opt():
    ...
    parser.add_argument('--phase', type=str, default='rgb', choices=['rgb', 'ir', 'multispectral'], help='validation phase')
