...
def create_dataloader_add_eoir(path,
                      imgsz,
                      batch_size,
                      stride,
                      single_cls=False,
    ...
    if rect and shuffle:
        ...
        shuffle = False
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadADDEOIRImagesAndLabels(
            path,
            imgsz,
            batch_size,
            augment=augment,  # augmentation
    ...
    return loader(dataset,
                  ...
                  sampler=sampler,
                  pin_memory=PIN_MEMORY,
                  collate_fn=LoadADDEOIRImagesAndLabels.collate_fn4 if quad else LoadADDEOIRImagesAndLabels.collate_fn,
                  worker_init_fn=seed_worker,
                  generator=generator), dataset
...
def create_dataloader_rgb_ir_add_eoir(path,
                      path_ir,
                      imgsz,
                      batch_size,
                      stride,
    ...
    if rect and shuffle:
        ...
        shuffle = False
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadMultispectralADDEOIRImagesAndLabels(
            path,
            path_ir,
            imgsz,
            batch_size,
    ...
    return loader(dataset,
                  ...
                  sampler=sampler,
                  pin_memory=PIN_MEMORY,
                  collate_fn=LoadMultispectralADDEOIRImagesAndLabels.collate_fn4 if quad \
                        else LoadMultispectralADDEOIRImagesAndLabels.collate_fn,
                  worker_init_fn=seed_worker,
                  generator=generator), dataset
...
class LoadADDEOIRImagesAndLabels(Dataset):
    # YOLOv5 train_loader/val_loader, loads images and labels for training and validation
    cache_version = 0.6  # dataset labels *.cache version
    rand_interp_methods = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA, cv2.INTER_LANCZOS4]
...
class LoadMultispectralADDEOIRImagesAndLabels(Dataset):
    # YOLOv5 train_loader/val_loader, loads images and labels for training and validation
    cache_version = 0.6  # dataset labels *.cache version
    rand_interp_methods = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA, cv2.INTER_LANCZOS4]
...
def verify_image_label_add_eoir(args):
    # Verify one image-label pair
    im_file, lb_file, prefix = args
    nm, nf, ne, nc, msg, segments = 0, 0, 0, 0, '', []  # number (missing, found, empty, corrupt), message, segments
    try:
